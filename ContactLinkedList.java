...
public class ContactLinkedList<T>{
    ...
    public void displayContactFound(String firstName){
		...
		while(current != null) {
			...
			if(fName.equalsIgnoreCase(firstName)){
				...
				displayContactFirstName(firstName);
	...
	public void displayContactFirstName(String firstName){
		
		Node<T> current = head;
		
		while(current != null) {
			
			String fName = ((Contact) current.getData()).getContactName();
			
			int ind = ((Contact) current.getData()).getContactName().indexOf(' ');
			
			if (ind >= 0)
				fName = ((Contact)current.getData()).getContactName().substring(0,ind);
